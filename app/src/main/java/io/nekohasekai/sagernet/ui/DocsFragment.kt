    ...
    private fun initDocData() {
        ...
        allItems.add(
            DocListItem.Item(
                ...
                badge = "核心工具",
                desc = "1:1 完整移植的官方纯正 sing-box 实时网络仪表盘。需在「设置 - 进阶设置」中开启「启用 Clash API」后，即可随时从侧边栏快捷进入。支持在页面内一键切换 Zashboard (现代推荐 · 界面精美)、内置 Yacd (官方离线轻量)、Metacubexd 或任意自定义 Web 面板链接；全面放行混合协议与跨域，自动注入本机 Clash API 参数直连。全面提供“概览、代理、规则、连接、配置、日志”多 Tab 导航视图，支持实时上传/下载流量动态图表、内核内存占用监控、策略组出站实时切换、活跃与历史连接多维过滤及一键断开等全套网络排查工具。",
                ...
                keywords = "仪表盘 仪表板 sing-box clash api zashboard yacd metacubexd 概览 代理 规则 活跃连接 连接 日志 内存 监控 抓包 menu_dashboard",
            )
        )
