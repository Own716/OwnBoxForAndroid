...
* **崩溃修复**：修复切换纯白模式后 `ConfigurationFragment` 初始化时序引发的 `NullPointerException` 空指针异常闪退。
* **架构防护**：重构 `Context.getColorAttr` 资源解析与容错机制，防御未找到资源 ID（`NotFoundException #0x0`）引发的布局解析失败。
* **视觉校准**：纯白模式强制反色深黑字与图标，彻底解决纯白界面字画同色（白纸盲显）缺陷。
* **组件适配**：底部统计栏（StatsBar）与悬浮服务按钮（FAB）深度适配黑白高对比度反色渲染。
* **系统稳定**：纯白模式自动避让 AMOLED 黑色样式与深色壁纸冲突，保障 App 重启与冷启动稳定流畅。
